...
class WalletHistoryViewController: UIViewController {
    ...
    override func viewDidLoad() {
       ...
       tableView.delegate = self
        ...
        titleLbl.text = Constants.string.walletHistory.localize().uppercased()
...
extension WalletHistoryViewController: PresenterOutputProtocol {
    
    func getSellhistory() {
        self.loader.isHidden = false
        self.presenter?.HITAPI(api: Base.walletHistory.rawValue, params: nil, methodType: .GET, modelClass: SellHistoryEntity.self, token: true)
    }
    ...
    func showSuccess(api: String, dataArray: [Mappable]?, dataDict: Mappable?, modelClass: Any) {
        ...
        case Base.walletHistory.rawValue:
            self.loader.isHidden = true
            self.sellHistoryDict = dataDict as? SellHistoryEntity
            self.historySell = self.sellHistoryDict?.walletHistory ?? []
