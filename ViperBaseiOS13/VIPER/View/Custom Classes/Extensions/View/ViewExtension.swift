...
extension UIView {
   ...
   //Set background color
    ...
    func addShadowDefault(cornerRadius: CGFloat, maskedCorners: CACornerMask = [.layerMaxXMinYCorner, .layerMinXMinYCorner, .layerMaxXMaxYCorner, .layerMinXMaxYCorner], color: UIColor, offset: CGSize, opacity: Float, shadowRadius: CGFloat) {
...
extension UIView {
    ...
    func applyEffectToView(borderColor: UIColor = #colorLiteral(red: 0.8705882353, green: 0.8705882353, blue: 0.8705882353, alpha: 1) ) {
        self.layer.masksToBounds = false
        self.layer.borderColor = borderColor.cgColor
        self.layer.borderWidth = 0.5
        self.backgroundColor = #colorLiteral(red: 0.9803921569, green: 0.9803921569, blue: 0.9843137255, alpha: 1)
        self.layer.cornerRadius = 5
    }
    
    
    func setCornerRadius(cornerR: CGFloat = 8) {
    ...
    func applyShadow(radius: CGFloat = 8.0) {
        
        self.layer.masksToBounds = false
        self.layer.shadowRadius = 2
        self.layer.shadowOpacity = 1
        self.layer.shadowColor = UIColor(hex: "#0000001A").cgColor.copy(alpha: 0.15)
    ...
    func setGradientBackgroundForView() {
              DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                  let gradientLayer = CAGradientLayer()
                  gradientLayer.frame = self.bounds
                  let color0 = UIColor(hex: "#E30000").cgColor
                  let color1 = UIColor(hex: "#CC4343").cgColor
    ...
    func setGradientBackgroundForCornerRadiusView() {
                 DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                     let gradientLayer = CAGradientLayer()
                     gradientLayer.frame = self.bounds
                     let color0 = UIColor(hex: "#E30000").cgColor
    ...
    func setGradientBackgroundForViewWithTrans() {
                 DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                     let gradientLayer = CAGradientLayer()
                     gradientLayer.frame = self.bounds
                    let color0 = UIColor(hex: "#E30000").cgColor.copy(alpha: 0.29)
                     let color1 = UIColor(hex: "#CC4343").cgColor.copy(alpha: 0.29)
                     gradientLayer.colors = [color0,color1]
                     gradientLayer.locations = [0.0,1.0]
                     gradientLayer.startPoint =  CGPoint(x: 0.0, y: 1.0)
                     gradientLayer.endPoint = CGPoint(x: 1.0, y: 1.0)
